...
for ((i=0; i < $NUM_MASTERS; i++)); do
  ...
  ssh ${SSH_USER}@${MASTER_HOSTNAMES[i]} sudo yum install -y yum-utils device-mapper-persistent-data lvm2
  if [ "$ARCH" == "ppc64le" ]; then
...
for ((i=0; i < $NUM_PROXYS; i++)); do
  ...
  ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo yum install -y yum-utils device-mapper-persistent-data lvm2
  if [ "$ARCH" == "ppc64le" ]; then
    ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} 'echo -e "[docker]\nname=Docker\nbaseurl=http://ftp.unicamp.br/pub/ppc64el/rhel/7/docker-ppc64el/\nenabled=1\ngpgcheck=0\n" | sudo tee /etc/yum.repos.d/docker.repo'
  else
    ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo yum-config-manager -y --add-repo https://download.docker.com/linux/centos/docker-ce.repo
  fi
  
  ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo yum update -y
  ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo yum install -y docker-ce

  # Fall back to pinned version (no fallback for ppc)
  if [ "$?" == "1" ]; then
    ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} yum install --setopt=obsoletes=0 -y docker-ce-17.03.2.ce-1.el7.centos.x86_64 docker-ce-selinux-17.03.2.ce-1.el7.centos.noarch
  fi
